...
class Base(unittest.TestCase):
    ...
    def running_task(self, task_id="BRIDGE-0900"):
    ...
    def audit_lines(self, task_id="BRIDGE-0900"):
...
class HeartbeatTests(Base):
    def test_beat_writes_schema_conform_and_read_back(self):
        doc = heartbeat.beat(self.tmp, "BRIDGE-0900", "RUN-01",
        ...
        self.assertEqual(
            heartbeat.read_heartbeat(self.tmp, "BRIDGE-0900", "RUN-01"), doc)

    def test_read_missing_returns_none(self):
        self.assertIsNone(
            heartbeat.read_heartbeat(self.tmp, "BRIDGE-0900", "RUN-01"))

    def test_beat_updates_last_seen(self):
        heartbeat.beat(self.tmp, "BRIDGE-0900", "RUN-01",
                       now=T0, schema_dir=SCHEMA_DIR)
        later = heartbeat.beat(self.tmp, "BRIDGE-0900", "RUN-01",
    ...
    def test_broken_heartbeat_file_raises(self):
        p = heartbeat.heartbeat_path(self.tmp, "BRIDGE-0900", "RUN-01")
...
class ScanResultTests(Base):
    def test_completed_result_scan_then_apply(self):
        ...
        self.assertEqual(self.store.load_task("BRIDGE-0900")["status"], "COMPLETED")
...
class ScanHeartbeatTests(Base):
    ...
    def test_missing_heartbeat_beyond_grace_flags_stale(self):
        self.running_task()
        (self.tmp / "results" / "BRIDGE-0900" / "RUN-01").mkdir(parents=True)
        started = watcher._run_started_at(self.store, "BRIDGE-0900")
...
class CliWatchTests(Base):
    ...
    def test_heartbeat_cli_writes_file(self):
        self.running_task()
        code, _, err = self.cli("watch", "heartbeat", "BRIDGE-0900", "RUN-01",
                                "--actor", "codex")
